    ...
    public class PropertyEditor : Editor
    {
        ...
        public virtual void AddSetupSettings(SerializedObject llmScriptSO)
        {
            ...
            if (remoteProperty != null)
                attributeClasses.Add(
                    remoteProperty.boolValue ? typeof(RemoteAttribute) : typeof(LocalAttribute)
                );
        ...
        {
            LLMUnitySetup.SetDebugMode(
                (LLMUnitySetup.DebugModeType)
                    EditorGUILayout.EnumPopup("Log Level", LLMUnitySetup.DebugMode)
            );
        }

        public void AddAdvancedOptionsToggle(SerializedObject llmScriptSO)
        {
            List<SerializedProperty> properties = GetPropertiesOfClass(
                llmScriptSO,
                new List<Type>() { typeof(AdvancedAttribute) }
            );
            ...
            string toggleText =
                (advancedOptionsProp.boolValue ? "Hide" : "Show") + " Advanced Options";
        ...
        public List<SerializedProperty> GetPropertiesOfClass(
            SerializedObject so,
            List<Type> attributeClasses
        )
        {
        ...
        public bool ShowPropertiesOfClass(
            string title,
            SerializedObject so,
            List<Type> attributeClasses,
            bool addSpace = true,
            List<Type> excludeAttributeClasses = null
        )
        {
            // display a property if it belongs to a certain class and/or has a specific attribute class
            ...
            {
                List<SerializedProperty> excludeProperties = GetPropertiesOfClass(
                    so,
                ...
                foreach (SerializedProperty excprop in excludeProperties)
                {
                    ...
                    {
                        if (prop.displayName == excprop.displayName)
                            removeProperties.Add(prop);
                    }
                }
                foreach (SerializedProperty prop in removeProperties)
            ...
            foreach (SerializedProperty prop in properties)
            {
                ...
                Attribute dynamicRangeAttr = GetPropertyAttribute(
                    prop,
        ...
        )
        {
            ...
            return field != null
                && field.DeclaringType == targetClass
                && (attributeClass == null || GetPropertyAttribute(prop, attributeClass) != null);
        }
        ...
        public Attribute GetPropertyAttribute(SerializedProperty prop, Type attributeClass)
        {
            ...
            foreach (var pathSegment in prop.propertyPath.Split('.'))
            {
                ...
                while (targetType != null)
                {
                    ...
                    {
                        foreach (
                            Attribute attr in fieldInfo.GetCustomAttributes(attributeClass, true)
                        )
                        {
