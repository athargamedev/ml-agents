    ...
    public class LLMAgentEditor : LLMCallerEditor
    {
        public override void AddModelSettings(SerializedObject llmScriptSO)
        {
            ...
            else
            {
                ...
                ShowPropertiesOfClass(
                    "",
                    llmScriptSO,
                    new List<Type> { typeof(ModelAttribute) },
                    false
                );
                ...
                if (GUILayout.Button("Load grammar", GUILayout.Width(buttonWidth)))
                {
                    EditorApplication.delayCall += () =>
                    {
                        string path = EditorUtility.OpenFilePanelWithFilters(
                            "Select a grammar file",
                            "",
                            new string[] { "Grammar Files", "json,gbnf" }
                        );
                ...
                ShowPropertiesOfClass(
                    "",
                    llmScriptSO,
                    new List<Type> { typeof(ModelAdvancedAttribute) },
                    false
                );
            }
        }
