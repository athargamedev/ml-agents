    ...
    public class LLMEditor : PropertyEditor
    {
        ...
        public void AddModelLoaders(SerializedObject llmScriptSO, LLM llmScript)
        {
            if (LLMManager.modelEntries.Count > 0)
            {
                ...
                foreach (float width in widths)
                    listWidth += width + (listWidth == 0 ? 0 : elementPadding);
                ...
                Rect expandedRect = GUILayoutUtility.GetRect(
                    actionColumnWidth,
                    modelList.elementHeight + ReorderableList.Defaults.padding
                );
                expandedRect.y +=
                    modelList.GetHeight()
                    - modelList.elementHeight
                    - ReorderableList.Defaults.padding;
            ...
            _ = AddLoadButtons();
            bool downloadOnStart = EditorGUILayout.Toggle(
                "Download on Start",
                LLMManager.downloadOnStart
            );
            if (downloadOnStart != LLMManager.downloadOnStart)
                LLMManager.SetDownloadOnStart(downloadOnStart);
        }
        ...
        static void ResetModelOptions()
        {
            ...
            foreach (var entry in LLMUnitySetup.modelOptions)
            {
                ...
                {
                    if (url != null && existingOptions.Contains(url))
                        continue;
        ...
        float[] GetColumnWidths(bool expandedView)
        {
            ...
            if (expandedView)
                widths.AddRange(new List<float>() { textColumnWidth, textColumnWidth });
        ...
        void SelectModel(string filename, bool lora, bool ifNoneSelected = true)
        {
            ...
            if (!lora)
                llmScript.SetModel(filename);
            if (lora)
                llmScript.AddLora(filename);
        }
        ...
        async Task createButtons()
        {
            ...
            int modelIndex = EditorGUILayout.Popup(
                0,
            ...
            else if (modelIndex > 1)
            {
                if (modelLicenses[modelIndex] != null)
                    LLMUnitySetup.LogWarning(
                        $"The {modelNames[modelIndex]} model is released under the following license: {modelLicenses[modelIndex]}. By using this model, you agree to the terms of the license."
                    );
                string filename = await LLMManager.DownloadModel(
        ...
        void OnEnable()
        {
            LLM llmScript = (LLM)target;
            ResetModelOptions();
            trashIcon = new GUIContent(
                Resources.Load<Texture2D>("llmunity_trash_icon"),
                "Delete Model"
            );
            ...
            modelList = new ReorderableList(
                LLMManager.modelEntries,
                typeof(ModelEntry),
                false,
                true,
                false,
            ...
            )
            {
                ...
                {
                    if (index >= LLMManager.modelEntries.Count)
                        return;
                    ...
                    if (!entry.lora)
                    {
                        ...
                        bool newSelected = EditorGUI.Toggle(
                            selectRect,
                            isSelected,
                            EditorStyles.radioButton
                        );
                        if (newSelected && !isSelected)
                            llmScript.SetModel(entry.filename);
                    }
                    else
                    {
                        ...
                        if (newSelected && !isSelected)
                            llmScript.AddLora(entry.filename);
                        else if (!newSelected && isSelected)
                            llmScript.RemoveLora(entry.filename);
                    }
                    ...
                    bool includeInBuild = EditorGUI.ToggleLeft(
                        includeInBuildRect,
                        "",
                    ...
                    if (
                        !entry.lora
                        && index < LLMManager.modelEntries.Count - 1
                        && LLMManager.modelEntries[index + 1].lora
                    )
                    {
                        GUI.DrawTexture(
                            new Rect(
        ...
        public override void AddSetupExtras()
        {
            ...
            GUIContent content = new GUIContent(
                "Light build for Nvidia GPUs",
                "Use tinyBLAS instead of cuBLAS that takes up less space and has similar performance for quants - doesn't work with i-quants and flash attention"
            );
            bool newUseCUBLAS = !EditorGUILayout.Toggle(content, !useCUBLAS);
            if (newUseCUBLAS != useCUBLAS)
                LLMUnitySetup.SetCUBLAS(newUseCUBLAS);
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUI.enabled =
                LLMUnitySetup.libraryProgress == 1
                && LLMManager.modelProgress == 1
                && LLMManager.loraProgress == 1;

            AddOptionsToggles(llmScriptSO);
            AddSetupSettings(llmScriptSO);
            if (llmScriptSO.FindProperty("_remote").boolValue)
