        ...
        public static void PostprocessIOSBuild(BuildTarget buildTarget, string outputPath)
        {
...
#elif UNITY_VISIONOS
            string projPath = PBXProject
                .GetPBXProjectPath(outputPath)
                .Replace("Unity-iPhone", "Unity-VisionOS");
            libraryFileNames.Add("libllamalib_visionos.a");
#else
            string projPath = Path.Combine(
                outputPath,
                Path.GetFileName(outputPath) + ".xcodeproj",
                "project.pbxproj"
            );
            ...
            else
            {
                foreach (string libraryFile in libraryFiles)
                {
                    ...
                    {
                        foreach (
                            var phaseGuid in project.GetAllBuildPhasesForTarget(unityMainTargetGuid)
                        )
                        {
