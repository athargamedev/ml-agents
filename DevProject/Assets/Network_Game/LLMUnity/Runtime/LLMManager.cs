...
namespace LLMUnity
{
    ...
    public class ModelEntry
    {
        ...
        static List<string> embeddingOnlyArchs = new List<string>
        {
            "bert",
            "nomic-bert",
            "jina-bert-v2",
    ...
    public class LLMManager
    {
        ...
        public static void SetDownloadProgress(float progress)
        {
            ...
            foreach (Action<float> downloadProgressCallback in downloadProgressCallbacks)
        ...
        public static async Task<bool> SetupOnce()
        {
            ...
            foreach (ModelEntry modelEntry in modelEntries)
            {
                ...
                if (!downloadOnStart || string.IsNullOrEmpty(modelEntry.url))
                {
                    ...
                    if (!File.Exists(target))
                        LLMUnitySetup.LogError($"Model {modelEntry.filename} could not be found!");
                }
                else
                {
        ...
        public static ModelEntry Get(string path)
        {
            ...
            {
                if (entry.filename == filename || entry.path == fullPath)
                    return entry;
            }
            return null;
        }
        ...
        public static void LoadFromDisk()
        {
            if (!File.Exists(LLMUnitySetup.LLMManagerPath))
                return;
            LLMManagerStore store = JsonUtility.FromJson<LLMManagerStore>(
                File.ReadAllText(LLMUnitySetup.LLMManagerPath)
            );
        ...
        public static string AddEntry(ModelEntry entry)
        {
            ...
            if (!entry.lora)
            {
                if (modelEntries.Count > 0 && modelEntries[0].lora)
                    indexToInsert = 0;
                else
                {
        ...
        /// <param name="label">model label</param>
        /// <returns>model filename</returns>
        public static async Task<string> DownloadLora(
            string url,
            bool log = false,
            string label = null
        )
        ...
        public static void Remove(ModelEntry entry)
        {
            ...
            {
                if (!entry.lora && llm.model == entry.filename)
                    llm.model = "";
                else if (entry.lora)
                    llm.RemoveLora(entry.filename);
            }
        }
        ...
        public static void Build(Action<string, string> copyCallback)
        {
            ...
            {
                string target = LLMUnitySetup.GetAssetPath(modelEntry.filename);
                if (!modelEntry.includeInBuild || File.Exists(target))
                    continue;
                if (!downloadOnStart || string.IsNullOrEmpty(modelEntry.url))
                    copyCallback(modelEntry.path, target);
            }
        }
