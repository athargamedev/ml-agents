...
namespace LLMUnity
{
    ...
    public class GGUFReader
    {
        ...
        private Dictionary<GGUFValueType, Type> gguf_scalar_to_np = new Dictionary<
            GGUFValueType,
            Type
        >
        {
        ...
        public GGUFReader(string path)
        {
            ...
            offs += PushField(
                new ReaderField
                {
                    offset = offs,
                    name = "GGUF.version",
                    parts = new List<Array> { new uint[] { temp_version } },
                    data = new List<int> { 0 },
                    types = new List<GGUFValueType> { GGUFValueType.UINT32 },
                }
            );
            ...
            offs += PushField(
                ...
                {
                    offset = offs,
                    name = "GGUF.tensor_count",
                    parts = new List<Array> { new ulong[] { temp_counts[0] } },
                    data = new List<int> { 0 },
                    types = new List<GGUFValueType> { GGUFValueType.UINT64 },
                }
            );
            offs += PushField(
                new ReaderField
                {
                    offset = offs,
                    name = "GGUF.kv_count",
                    parts = new List<Array> { new ulong[] { temp_counts[1] } },
                    data = new List<int> { 0 },
                    types = new List<GGUFValueType> { GGUFValueType.UINT64 },
                }
            );
        ...
        public byte[] GetGenericField(string key)
        {
            ...
            if (field == null || field.parts.Count == 0)
                return null;
        ...
        private (int, List<Array>, List<int>, List<GGUFValueType>) GetFieldParts(
            int orig_offs,
            int raw_type
        )
        {
            ...
            if ((GGUFValueType)raw_type == GGUFValueType.ARRAY)
            {
                ...
                List<Array> aparts = new List<Array>
                {
                    BitConverter.GetBytes(raw_itype),
                    BitConverter.GetBytes(alen),
                };
                ...
                for (int idx = 0; idx < (int)alen; idx++)
                {
                    (
                        int curr_size,
                        List<Array> curr_parts,
                        List<int> curr_idxs,
                        List<GGUFValueType> curr_types
                    ) = GetFieldParts(offs, raw_itype);
                    if (idx == 0)
        ...
        private int BuildFields(int offs, int count)
        {
            for (int i = 0; i < count; i++)
            {
                ...
                offs += Marshal.SizeOf(typeof(int));
                List<Array> parts = new List<Array>
                {
                    kv_klen,
                    kv_kdata,
                    BitConverter.GetBytes(raw_kv_type),
                };
                List<int> idxs_offs = new List<int> { parts.Count };

                (
                    int field_size,
                    List<Array> field_parts,
                    List<int> field_idxs,
                    List<GGUFValueType> field_types
                ) = GetFieldParts(offs, raw_kv_type);
                if (field_size == -1)
                    continue;
