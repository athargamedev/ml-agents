...
namespace LLMUnity
{
    ...
    public class LLMException : Exception
    {
        ...
        public LLMException(string message, int errorCode)
            : base(message)
        {
            ErrorCode = errorCode;
        }
    ...
    public class LoraManager
    {
        ...
        public int IndexOf(string path)
        {
            ...
            for (int i = 0; i < loras.Count; i++)
            {
                ...
                if (lora.assetPath == path || lora.fullPath == fullPath)
                    return i;
            }
            return -1;
        }
        ...
        public void FromStrings(string loraString, string loraWeightsString)
        {
            ...
            try
            {
                ...
                List<string> loraWeightsStringArr = new List<string>(
                    loraWeightsString.Split(delimiter)
                );
                ...
                for (int i = 0; i < loraStringArr.Count; i++)
                    lorasNew.Add(
                        new LoraAsset(loraStringArr[i].Trim(), float.Parse(loraWeightsStringArr[i]))
                    );
                loras = lorasNew;
            }
            catch (Exception e)
