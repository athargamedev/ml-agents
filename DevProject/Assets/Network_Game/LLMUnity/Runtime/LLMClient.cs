...
namespace LLMUnity
{
    ...
    public class LLMClient : MonoBehaviour
    {
        ...
        [ModelAdvanced, TextArea(1, 10), SerializeField]
        ...
        protected virtual async Task CheckCaller(bool checkConnection = true)
        {
            ...
            if (GetCaller() == null)
                LLMUnitySetup.LogError("LLM caller not initialized", true);
            if (remote && checkConnection)
            {
                for (int attempt = 0; attempt <= numRetries; attempt++)
                {
                    if (llmClient.IsServerAlive())
                        break;
                    await Task.Yield();
                }
            }
            ...
            try
            {
                if (!remote)
                {
                    ...
                    if (llm?.llmService == null)
                        LLMUnitySetup.LogError("Local LLM service is not available", true);
        ...
        public virtual async Task<List<int>> Tokenize(
            string query,
        ...
        public virtual async Task<string> Detokenize(
            List<int> tokens,
        ...
        public virtual async Task<List<float>> Embeddings(
            string query,
        ...
        )
        {
            ...
            if (embeddings.Count == 0)
                LLMUnitySetup.LogError("embeddings are empty!");
        ...
        public virtual async Task<string> Completion(
            string prompt,
            Action<string> callback = null,
