...
namespace LLMUnity
{
    ...
    public class LLMBuilder : AssetPostprocessor
    {
        ...
        public static string BuildTempDir = Path.Combine(
        ...
        public static string PluginDir(string platform, bool relative = false)
        {
            ...
            if (!relative)
                pluginDir = Path.Combine(Application.dataPath, pluginDir);
            return pluginDir;
        }
        ...
        public static void Retry(System.Action action, int retries = 10, int delayMs = 100)
        {
            for (int i = 0; i < retries; i++)
            {
                ...
                {
                    if (i == retries - 1)
                        LLMUnitySetup.LogError(e.Message, true);
        ...
        public static void HandleActionFileRecursive(
            string source,
            string target,
        ...
        )
        {
            if (File.Exists(source))
            {
                ...
                if (targetDir != "" && !Directory.Exists(targetDir))
            ...
            else if (Directory.Exists(source))
            {
                ...
                foreach (string path in filesAndDirs)
                {
                    HandleActionFileRecursive(
                        path,
                        Path.Combine(target, Path.GetFileName(path)),
                        actionCallback
                    );
                }
            }
        ...
        public static bool DeletePath(string path)
        {
            ...
            bool deleteOK = false;
            foreach (string allowedDir in allowedDirs)
                deleteOK = deleteOK || LLMUnitySetup.IsSubPath(path, allowedDir);
            if (!deleteOK)
            {
        ...
        static void AddMovedPair(string source, string target)
        {
            ...
            File.WriteAllText(
                movedCache,
                JsonUtility.ToJson(new ListStringPair { pairs = movedPairs }, true)
            );
        }
        ...
        static bool MoveAction(string source, string target, bool addEntry = true)
        {
            ...
            if (File.Exists(source) || Directory.Exists(source))
                moveCallback = MovePath;
            else
                return false;

            if (addEntry)
                AddMovedPair(source, target);
        ...
        static bool CopyAction(string source, string target, bool addEntry = true)
        {
            ...
            if (File.Exists(source))
                copyCallback = File.Copy;
            else if (Directory.Exists(source))
                copyCallback = CopyPath;
            else
                return false;
        ...
        public static void BuildLibraryPlatforms(BuildTarget buildTarget)
        {
            ...
            if (checkCUBLAS)
            {
                List<string> exclusionKeywords = LLMUnitySetup.CUBLAS
                    ? new List<string>() { "tinyblas" }
                    : new List<string>() { "cublas", "cudart" };
                foreach (string platform in platforms)
                {
                    string platformDir = Path.Combine(
                        LLMUnitySetup.libraryPath,
                        platform,
                        "native"
                    );
        ...
        )
        {
            ...
            foreach (BuildTarget buildTarget in buildTargets)
            {
                string platformDir = Path.Combine(
                    "Assets",
                    PluginDir(buildTarget.ToString(), true)
                );
                ...
                foreach (string archDir in Directory.GetDirectories(platformDir))
                {
                    ...
                    string pathToPlugin = Path.Combine(
                        "Assets",
                        MobilePluginPath(buildTarget, arch, true)
                    );
        ...
        public static void BuildModels()
        {
            ...
            if (File.Exists(LLMUnitySetup.LLMManagerPath))
                AddActionAddMeta(LLMUnitySetup.LLMManagerPath);
        }
        ...
        {
            if (!File.Exists(movedCache))
                return;
            List<StringPair> movedPairs = JsonUtility
                .FromJson<ListStringPair>(File.ReadAllText(movedCache))
                .pairs;
            if (movedPairs == null)
                return;
