...
namespace LLMUnity
{
    ...
    {
        public LLMUnityException(string message = "")
            : base(message) { }
    }
    ...
    public class LLMUnitySetup
    {
        ...
        public static string LlamaLibReleaseURL =
        ...
        public static string LLMUnityStore = Path.Combine(
        ...
        [HideInInspector]
        public static readonly Dictionary<string, (string, string, string)[]> modelOptions =
        ...
        {
            if ((int)DebugMode > (int)DebugModeType.All)
                return;
            Debug.Log(message);
        }
        ...
        {
            if ((int)DebugMode > (int)DebugModeType.Warning)
                return;
        ...
        {
            if ((int)DebugMode > (int)DebugModeType.Error)
                return;
            Debug.LogError(message);
            foreach (Action<string> errorCallback in errorCallbacks)
        ...
        static Dictionary<string, ResumingWebClient> downloadClients =
            new Dictionary<string, ResumingWebClient>();

        public static void CancelDownload(string savePath)
        {
            if (!downloadClients.ContainsKey(savePath))
                return;
        ...
        )
        {
            ...
            else
            {
                ...
                await client.DownloadFileTaskAsyncResume(
                    new Uri(fileUrl),
                    tmpPath,
                    !overwrite,
                    progressCallback
        ...
        public static async Task AndroidExtractFile(
            string assetName,
            bool overwrite = false,
            bool log = true,
        ...
        public static async Task AndroidExtractFileOnce(
            string assetName,
            bool overwrite = false,
            bool log = true,
            int chunkSize = 1024 * 1024
        ...
        {
            return GetFullPath(childPath)
                .StartsWith(GetFullPath(parentPath), StringComparison.OrdinalIgnoreCase);
        }
        ...
        static void ExtractInsideDirectory(
            string zipPath,
            string extractPath,
            string prefix = "",
            bool overwrite = true
        ...
        static void DeleteEarlierVersions()
        {
            ...
            )
            {
                if (Directory.Exists(dir))
                    assetPathSubDirs.AddRange(Directory.GetDirectories(dir));
            }

            List<Regex> versionRegexes = new List<Regex>
            {
                new Regex("undreamai-(.+)-llamacpp"),
                new Regex("LlamaLib-(.+)"),
            };
                ...
                foreach (Regex regex in versionRegexes)
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        if (version != LlamaLibVersion)
                        {
                            ...
                            if (File.Exists(assetPathSubDir + ".meta"))
                                File.Delete(assetPathSubDir + ".meta");
                        }
                    }
                }
            }
        ...
        public static string AddAsset(string assetPath)
        {
            ...
            if (IsSubPath(assetPath, assetDir))
        ...
        public static int AndroidGetNumBigCores()
        {
                ...
                if (Directory.Exists(cpuPath))
                {
                    foreach (string cpuDir in Directory.GetDirectories(cpuPath))
                    {
                        ...
                        if (!dirName.StartsWith("cpu"))
                            continue;
                        if (!int.TryParse(dirName.Substring(3), out coreIndex))
                            continue;

                        int maxFreqKHz = GetMaxFreqKHz(coreIndex);
                        cpuMaxFreqKHz.Add(maxFreqKHz);
                        if (maxFreqKHz > maxFreqKHzMax)
                            maxFreqKHzMax = maxFreqKHz;
                        if (maxFreqKHz < maxFreqKHzMin)
                            maxFreqKHzMin = maxFreqKHz;
            ...
            if (maxFreqKHzMedium == maxFreqKHzMax)
                numBigCores = numCores;
            else
            {
                for (int i = 0; i < cpuMaxFreqKHz.Count; i++)
                {
                    if (cpuIsSmtCpu[i] || cpuMaxFreqKHz[i] >= maxFreqKHzMedium)
                        numBigCores++;
                }
            }

            if (numBigCores == 0)
                numBigCores = SystemInfo.processorCount / 2;
            else
                numBigCores = Math.Min(numBigCores, SystemInfo.processorCount);
        ...
        public static int AndroidGetNumBigCoresCapacity()
        {
            ...
            if (numBigCores == 0 || numBigCores > SystemInfo.processorCount)
                numBigCores = SystemInfo.processorCount;
            return numBigCores;
        }
    }
}
