...
namespace LLMUnity
{
    ...
    public class ResumingWebClient : WebClient
    {
        ...
        public Task DownloadFileTaskAsyncResume(
            Uri address,
            string fileName,
            bool resume = false,
        ...
        )
        {
            try
            {
                ...
                byte[] copyBuffer = new byte[
                    contentLength == -1 || contentLength > DefaultDownloadBufferLength
                ...
                using (Stream readStream = response.GetResponseStream())
                {
                    if (readStream != null)
                    {
                        while (true)
                        {
                            int bytesRead = await readStream
                                .ReadAsync(new Memory<byte>(copyBuffer))
                            ...
                            await writeStream
                                .WriteAsync(new ReadOnlyMemory<byte>(copyBuffer, 0, bytesRead))
                                .ConfigureAwait(false);
                        }
                    }
        ...
        )
        {
            if (progressCallback != null && BytesReceived > 0)
            {
                float progressPercentage =
                    TotalBytesToReceive < 0 ? 0
                    : TotalBytesToReceive == 0 ? 1
                    : (float)BytesReceived / TotalBytesToReceive;
